  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-10">
        {/* Already rendering heading from WorkTimeline parent */}
      </div>
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-4xl lg:text-5xl font-bold text-muted-foreground/40">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-lg mb-3 text-left font-bold text-muted-foreground/40">
                {item.title}
              </h3>
              {item.content}
            </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-[2px] bg-gradient-to-t from-[hsl(var(--color-secondary))] via-[hsl(var(--color-accent))] to-transparent rounded-full"
          />
        </div>
      </div>
    </div>
